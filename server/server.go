...
func checkRawQuery(r *http.Request, want string) error {
	...
	if reflect.DeepEqual(rQuery, wantQuery) == false {
		return errors.New(fmt.Sprintf("Request query is not equal : %v, want query is %v", rQuery, wantQuery))
	}
	return nil
...
func (rm *RouterMap) Handler(w http.ResponseWriter, r *http.Request) {
	...
	if !isMatched {
		http.Error(w, fmt.Sprintf(`{"message": there is no matched handler for request:%s, "error:" %s}`, r.URL, err.Error()), 400)
	}
	return
